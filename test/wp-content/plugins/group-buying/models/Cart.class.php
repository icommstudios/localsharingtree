...
class Group_Buying_Cart extends Group_Buying_Post_Type {
	...
	public static function init() {
		...
		add_action( 'before_delete_post', array( __CLASS__, 'clear_id_cache_on_delete' ), 10, 1 );
	}
	...
	public static function get_instance( $user_id = 0 ) {
		...
		if ( !isset( self::$instances[$cart_id]->post->post_type ) )
			return NULL;
	...
	public static function clear_id_cache_on_delete( $post_id ) {
		if ( get_post_type($post_id) == self::POST_TYPE ) {
			$user_id = get_post_meta( $post_id, self::$meta_keys['user_id'], TRUE );
			if ( !empty($user_id) ) {
				self::clear_id_cache($user_id);
			}
		}
	}
