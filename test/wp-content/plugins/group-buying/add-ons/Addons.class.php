...
class Group_Buying_Addons extends Group_Buying_Controller {
	...
	private static function addon_definitions() {
		...
		self::$addons['attributes'] = array(
			...
			'files' => array(
				...
				GB_PATH.'/template-tags/add-ons/attributes.php',
			),
			'callbacks' => array(
		...
		self::$addons['gifts'] = array(
			...
			'files' => array(
				...
				GB_PATH.'/template-tags/add-ons/gifts.php',
			),
			'callbacks' => array(
				array( 'Group_Buying_Gift', 'init' ),
				array( 'Group_Buying_Gifts', 'init' ),
			),
		);
		...
		self::$addons['syndication_service'] = array(
		...
		self::$addons['gbs_importer_framework'] = array(
			...
			'files' => array(
				...
				GB_PATH.'/add-ons/importer/GBS_Importer_API.php',
			),
			'callbacks' => array(
				array( 'GBS_Importer_Framework', 'init' ),
			),
		);
	...
	public static function display_addons_options() {
		...
		foreach ( $addons as $key => $details ) {
			$id = self::ADDONS_SETTING.'['.$key.']';
			$title = ( isset( $details['url'] ) && $details['url'] != '' ) ? '<h2><a href="'.$details['url'].'" target="_blank">'.$details['label'].'</a></h2>' : '<h2>'.$details['label'].'</h2>' ;
			$support_link = ( isset( $details['support_url'] ) && $details['support_url'] != '' ) ? '<span class="addon_support_link"><a href="'.$details['support_url'].'" target="_blank">'.self::__('Support').'</a></span>' : '';
			printf( '<span class="check_slider"><input type="checkbox" name="%s" id="%s" value="%s" %s /> <label for="%s" ></label></span> %s<p class="description">%s</p> %s', $id, $id, $key, checked( TRUE, self::is_enabled( $key ), FALSE ), $id, $title, $details['description'], $support_link );
		}
	}
