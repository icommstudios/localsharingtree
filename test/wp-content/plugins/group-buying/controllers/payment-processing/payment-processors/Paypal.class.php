...
class Group_Buying_Paypal_EC extends Group_Buying_Offsite_Processors {
	...
	public function send_offsite( Group_Buying_Checkouts $checkout ) {
		...
		if ( !isset( $_GET['token'] ) && $_REQUEST['gb_checkout_action'] == Group_Buying_Checkouts::PAYMENT_PAGE ) {
			...
			do_action( 'gb_log', __CLASS__ . '::' . __FUNCTION__ . ' - PayPal EC Approval Response', $response );
			...
			do_action( 'gb_log', __CLASS__ . '::' . __FUNCTION__ . ' - PayPal EC Approval Response (Parsed)', $response );
	...
	private function set_nvp_data( Group_Buying_Checkouts $checkout ) {
		...
		do_action( 'gb_log', __CLASS__ . '::' . __FUNCTION__ . ' - PayPal EC SetCheckout Data', $nvpData );
	...
	public function process_payment( Group_Buying_Checkouts $checkout, Group_Buying_Purchase $purchase ) {
		...
		do_action( 'gb_log', __CLASS__ . '::' . __FUNCTION__ . ' - PayPal EC Authorization Response (Raw)', $response );
		...
		$response = wp_parse_args( wp_remote_retrieve_body( $response ) );
		do_action( 'gb_log', __CLASS__ . '::' . __FUNCTION__ . ' - PayPal EC Authorization Response (Parsed)', $response );
	...
	private function create_recurring_payment_profile( $item, Group_Buying_Checkouts $checkout, Group_Buying_Purchase $purchase ) {
		...
		do_action( 'gb_log', __CLASS__ . '::' . __FUNCTION__ . ' - PayPal EC Recurring Payment Response (Raw)', $response );
		...
		do_action( 'gb_log', __CLASS__ . '::' . __FUNCTION__ . ' - PayPal EC Recurring Payment Response (Parsed)', $response );
	...
	private function get_recurring_payment_status( $profile_id, Group_Buying_Payment $payment ) {
		...
		do_action( 'gb_log', __CLASS__ . '::' . __FUNCTION__ . ' - PayPal EC Recurring Payment Details Response (Raw)', $response );
		...
		do_action( 'gb_log', __CLASS__ . '::' . __FUNCTION__ . ' - PayPal EC Recurring Payment Details Response (Parsed)', $response );
	...
	public function cancel_recurring_payment( Group_Buying_Payment $payment ) {
		...
		do_action( 'gb_log', __CLASS__ . '::' . __FUNCTION__ . ' - PayPal EC Cancel Recurring Payment Response (Raw)', $response );
	...
	private function set_error_messages( $message, $display = TRUE ) {
		if ( $display ) {
			...
			do_action( 'gb_error', __CLASS__ . '::' . __FUNCTION__ . ' - error message from paypal', $message );
		}
	}
}
