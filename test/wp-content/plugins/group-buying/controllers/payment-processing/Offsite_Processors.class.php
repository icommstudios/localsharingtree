...
class Group_Buying_Offsite_Processor_Handler extends Group_Buying_Controller {
	...
	public static function register_registration_callback( GB_Router $router ) {
		$args = array(
			'path' => self::$payment_handler_path,
			'title' => 'Payment Handler',
			'page_callback' => array( get_class(), 'on_handler_page' )
		);
		$router->add_route( self::PAYMENT_HANDLER_QUERY_ARG, $args );
	}
	...
	public static function get_url() {
		if ( self::using_permalinks() ) {
			return trailingslashit( home_url( '', self::$use_ssl?'https':NULL ) ).trailingslashit( self::$payment_handler_path );
		} else {
			$router = GB_Router::get_instance();
			return $router->get_url( self::PAYMENT_HANDLER_QUERY_ARG );
		}
	}
	...
	public static function on_handler_page() {
		$offsite_handler = self::get_instance();
		// View template
		$offsite_handler->handle_action();
	}
