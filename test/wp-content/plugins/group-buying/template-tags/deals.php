...
function gb_get_deal_purchasers( $post_id = 0, $user_ids = FALSE  ) {
	...
	return apply_filters( 'gb_get_deal_purchasers', $ids, $post_id, $user_ids );
}
...
function gb_get_last_viewed_redirect_url( $last_viewed = null ) {
	...
	if ( isset( $_GET['redirect_to'] ) ) {
		...
		$url = home_url( trailingslashit( $post_type->rewrite['slug'] ) . $last_viewed );
	} elseif ( isset( $_POST['deal_redirect'] ) ) {
		$post_type = get_post_type_object( gb_get_deal_post_type() );
		$url = home_url( trailingslashit( $post_type->rewrite['slug'] ) . $_POST['deal_redirect'] );
	} else {
 ...
 * @return boolean
 */
function gb_has_purchased( $post_id = 0, $user_id = 0 ) {
	$bool = FALSE;
	if ( !$post_id ) {
		global $post;
		$post_id = $post->ID;
