...
/**
 * Get the Account Post Type
 * @return string
 */
function gb_get_account_post_type() {
	return Group_Buying_Account::POST_TYPE;
}
...
function gb_on_account_page() {
	return Group_Buying_Accounts::is_account_page();
}
...
function gb_on_account_edit_page() {
	return Group_Buying_Accounts_Edit_Profile::is_account_edit_page();
}
...
function gb_on_login_page() {
	return Group_Buying_Accounts_Login::is_login_page();
}
...
function gb_on_registration_page() {
	return Group_Buying_Accounts_Registration::is_registration_page();
}
...
function gb_on_reset_password_page() {
	return Group_Buying_Accounts_Retrieve_Password::is_rp_page();
}
...
function gb_get_purchased_deals( $user_id = 0, $return_array = FALSE ) {
	if ( !$user_id ) {
		$user_id = get_current_user_id();
	}
...
function gb_get_user_email( $user_id = 0 ) {
	...
	if ( !is_a( $user, 'WP_User' ) ) {
		do_action( 'gb_error', 'gb_get_user_email() - Get User Email FAILED', $user );
	}
