...
function gb_on_merchant_dashboard_page() {
	return Group_Buying_Merchants_Dashboard::is_merchant_dash();
}
...
function gb_on_deal_submit_page() {
	return Group_Buying_Deals_Submit::is_submit_page();
}
...
function gb_get_merchant_type_url() {
	return apply_filters( 'gb_get_merchant_type_url', home_url( trailingslashit( Group_Buying_Merchant::MERCHANT_TYPE_TAX_SLUG ) ) );
}
...
function gb_get_merchant_total_sold( $merchant_id = null, $refresh = false ) {
	...
	$merch_deals = get_posts( $args );
	if ( !empty( $merch_deals ) ) {
		foreach ( $merch_deals as $deal_id ) {
			$count += (int) gb_get_number_of_purchases( $deal_id );
		}
	}
