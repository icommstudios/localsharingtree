...
function origin_widgets_init(){
	foreach(glob(SITEORIGIN_PANELS_PATH.'/widgets/widgets/*/*.php') as $file) {
		include_once ($file);
...
abstract class SiteOrigin_Panels_Widget extends WP_Widget{
	...
	function create_css($style, $preset) {
		...
		if(!class_exists('lessc')) include SITEORIGIN_PANELS_PATH.'/widgets/lib/lessc.inc.php';
		...
		$less = str_replace(
			'@import "../../../less/mixins";',
			"\n\n".file_get_contents(SITEORIGIN_PANELS_PATH.'/widgets/less/mixins.less'),
			$less
		);
