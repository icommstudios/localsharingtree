...
define ('SITEORIGIN_PANELS_PATH', get_template_directory() . SEC_ADDONS_DIR . '/' . basename( dirname( __FILE__ ) ) );
define ('SITEORIGIN_PANELS_URL', get_template_directory_uri() . SEC_ADDONS_DIR . '/' . basename( dirname( __FILE__ ) ) );

include SITEORIGIN_PANELS_PATH . '/widgets/basic.php';

include SITEORIGIN_PANELS_PATH . '/inc/options.php';
include SITEORIGIN_PANELS_PATH . '/inc/revisions.php';
include SITEORIGIN_PANELS_PATH . '/inc/copy.php';
include SITEORIGIN_PANELS_PATH . '/inc/styles.php';
include SITEORIGIN_PANELS_PATH . '/inc/legacy.php';
include SITEORIGIN_PANELS_PATH . '/inc/notice.php';

if( defined('SITEORIGIN_PANELS_DEV') && SITEORIGIN_PANELS_DEV ) include SITEORIGIN_PANELS_PATH.'/inc/debug.php';
...
function siteorigin_panels_init(){
	...
	if( !defined('SITEORIGIN_PANELS_LEGACY_WIDGETS_ACTIVE') && ( !is_admin() || basename($_SERVER["SCRIPT_FILENAME"]) != 'plugins.php') ) {
		...
		include SITEORIGIN_PANELS_PATH.'/widgets/widgets.php';
	}
}
...
function siteorigin_panels_render_admin_home_page(){
	...
	include SITEORIGIN_PANELS_PATH.'/tpl/admin-home-page.php';
}
...
function siteorigin_panels_metabox_render( $post ) {
	include SITEORIGIN_PANELS_PATH.'/tpl/metabox-panels.php';
}
...
function siteorigin_panels_admin_enqueue_scripts($prefix) {
	...
	if ( ( $screen->base == 'post' && in_array( $screen->id, siteorigin_panels_setting('post-types') ) ) || $screen->base == 'appearance_page_so_panels_home_page') {
		...
		wp_enqueue_script( 'so-undomanager', SITEORIGIN_PANELS_URL . '/js/undomanager.min.js', array( ), 'fb30d7f' );
		wp_enqueue_script( 'so-panels-chosen', SITEORIGIN_PANELS_URL . '/js/chosen/chosen.jquery.min.min.js', array( 'jquery' ), SITEORIGIN_PANELS_VERSION );

		wp_enqueue_script( 'so-panels-admin', SITEORIGIN_PANELS_URL . '/js/panels.admin.min.js', array( 'jquery' ), SITEORIGIN_PANELS_VERSION );
		wp_enqueue_script( 'so-panels-admin-panels', SITEORIGIN_PANELS_URL . '/js/panels.admin.panels.min.js', array( 'jquery' ), SITEORIGIN_PANELS_VERSION );
		wp_enqueue_script( 'so-panels-admin-grid', SITEORIGIN_PANELS_URL . '/js/panels.admin.grid.min.js', array( 'jquery' ), SITEORIGIN_PANELS_VERSION );
		wp_enqueue_script( 'so-panels-admin-prebuilt', SITEORIGIN_PANELS_URL . '/js/panels.admin.prebuilt.min.js', array( 'jquery' ), SITEORIGIN_PANELS_VERSION );
		wp_enqueue_script( 'so-panels-admin-tooltip', SITEORIGIN_PANELS_URL . '/js/panels.admin.tooltip.min.js', array( 'jquery' ), SITEORIGIN_PANELS_VERSION );
		wp_enqueue_script( 'so-panels-admin-media', SITEORIGIN_PANELS_URL . '/js/panels.admin.media.min.js', array( 'jquery' ), SITEORIGIN_PANELS_VERSION );
		wp_enqueue_script( 'so-panels-admin-styles', SITEORIGIN_PANELS_URL . '/js/panels.admin.styles.min.js', array( 'jquery' ), SITEORIGIN_PANELS_VERSION );
...
function siteorigin_panels_admin_enqueue_styles() {
	...
	if ( in_array( $screen->id, siteorigin_panels_setting('post-types') ) || $screen->base == 'appearance_page_so_panels_home_page') {
		wp_enqueue_style( 'so-panels-admin', SITEORIGIN_PANELS_URL . '/css/admin.css', array( ), SITEORIGIN_PANELS_VERSION );

		global $wp_version;
		if( version_compare( $wp_version, '3.9.beta.1', '<' ) ) {
			// Versions before 3.9 need some custom jQuery UI styling
			wp_enqueue_style( 'so-panels-admin-jquery-ui', SITEORIGIN_PANELS_URL . '/css/jquery-ui.css', array(), SITEORIGIN_PANELS_VERSION );
		}
		else {
			wp_enqueue_style( 'wp-jquery-ui-dialog' );
		}

		wp_enqueue_style( 'so-panels-chosen', SITEORIGIN_PANELS_URL . '/js/chosen/chosen.css', array(), SITEORIGIN_PANELS_VERSION );
...
function siteorigin_panels_add_help_tab_content(){
	include SITEORIGIN_PANELS_PATH . '/tpl/help.php';
}
...
function siteorigin_panels_enqueue_styles(){
	wp_register_style('siteorigin-panels-front', SITEORIGIN_PANELS_URL . '/css/front.css', array(), SITEORIGIN_PANELS_VERSION );
}
