...
<div class="dashboard_container section main_block">
	...
	// Purchase history
		if ( $account_merchant_id && is_array( $merchants_deal_ids ) ) {
			...
			$merchant_deal_total_sales = 0; //start total merchant's deal sales at 0
			if ($deals->have_posts()) {
				?>
				...
				<table class="report_table merchant_dashboard"><!-- Begin .purchase-table -->
					...
					<thead>
						<tr>
                           ...
                           <th class="th_total_sold"><?php gb_e('Total Sales'); ?></th>
					...
					<?php
				while ($deals->have_posts()) : $deals->the_post();
					//Total sales for deal
					$purchase_ids = Group_Buying_Purchase::get_purchases( array( 'deal' => get_the_ID() ) );
					$deal_total_sales = 0;
					if ( $purchase_ids ) {
						foreach ($purchase_ids as $purchase_id ) {
							$purchase = Group_Buying_Purchase::get_instance( $purchase_id );
							$deal_purchase_qty = $purchase->get_product_quantity( get_the_ID() );
							$deal_purchase_unit_price = $purchase->get_product_unit_price( get_the_ID() );
							$deal_total_sales += $deal_purchase_qty * $deal_purchase_unit_price;
						}
					}
					//Add to running merchant total
					$merchant_deal_total_sales += $deal_total_sales;
					?>
						<tr>
                           ...
                           <td class="td_total_sales"><?php echo gb_get_formatted_money( $deal_total_sales ); ?></td>
                ...
                <div>
                <h4>Total Deal Sales: <?php echo gb_get_formatted_money($merchant_deal_total_sales); ?></h4>
                </div>
