...
class PremiumPressTheme_Payment {
	...
	function CustomGateway($gateway=""){
		...
		if(isset($_POST['customgateway']) && $_POST['customgateway'] == "payTrace" ){ 
		
		// INCLUDE API
		require_once(TEMPLATEPATH.'/PPT/gateways/class_paytrace.php');
				
		$errors = array();	
		// Check required fields for blanks
		// NOTE: you will need to sanitize these form values first!
		
		if (empty($_POST['cc_num'])) {	$errors[] = 'credit card number';	} else {	$cc_num = $_POST['cc_num'];	}
		if (empty($_POST['exp_month'])) {$errors[] = 'expiration month';} else {$exp_month = $_POST['exp_month'];	}	
		if (empty($_POST['exp_year'])) {$errors[] = 'expiration year';} else {$exp_year = $_POST['exp_year'];	}	
		if (empty($_POST['amount'])) {	$errors[] = 'amount';} else {	$amount = $_POST['amount'];	}	
		if (empty($_POST['csc'])) {		$errors[] = 'CSC';	} else {$csc = $_POST['csc'];	}	
		if (empty($_POST['street'])) {	$errors[] = 'billing street';	} else {		$street = $_POST['street'];	}	
		if (empty($_POST['zip'])) {		$errors[] = 'billing zip';	} else {		$zip = $_POST['zip'];	}
		...
		$PayTraceAPI->SetUN(get_option('paytrace_u'));
		$PayTraceAPI->SetPSWD(get_option('paytrace_p'));
		...
		if ( $PayTraceAPI->WasTransactionApproved() == true ) { 
				...
				$this->UpdateOrderStatus(5, $_POST['orderid']);	
	...
	**/
	...
	function PaymentStatus($orderID=""){
		...
		if(isset($_GET['order_id']) && $userdata->ID == 11){ // TEST FOR ADMIN
		...
		}elseif(isset($_POST['HashDigest'])){ // CARDSAVE
		
			if($_POST['StatusCode'] != 0 && $_POST['StatusCode'] != 20){
			return "error";
			}
			...
			if(is_numeric($emailID) && $emailID != 0){	
				...
				$result = mysql_query("SELECT order_data FROM ".$wpdb->prefix."orderdata WHERE ".$wpdb->prefix."orderdata.order_id='".strip_tags(PPTCLEAN($_POST['OrderID']))."' LIMIT 1", $wpdb->dbh) 
				or die(mysql_error().' on line: '.__LINE__);							
				$array = mysql_fetch_assoc($result);				
	...
	function UpdateOrderStatus($status, $order_id){
		...
		foreach($_POST as $key=>$val){ if(is_array($val)){ foreach($val as $key1=>$val1){ if($val1 !=""){ $POSTDATA .= $key1."=".$val1."\n"; } } }else{ if($val1 !=""){ $POSTDATA .= $key."=".$val."\n"; } } }
...
global $wpdb;
	...
	if($oData["SAVE"]){
		...
		if($key == 'paydata'){
			...
			$POSTDATA = "";
			foreach($_POST as $key=>$val){ if(is_array($val)){ foreach($val as $key1=>$val1){ if($val1 !="" && !is_array($val1) ){ $POSTDATA .= $key1."=".$val1."\n"; } } }else{ if($val1 !=""){ $POSTDATA .= $key."=".$val."\n"; } } }
			$oData['paydata'] = mysql_real_escape_string($POSTDATA);	
