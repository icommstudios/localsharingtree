...
class PremiumPressTheme_Datafeedr {
	...
	global $PPT,$wpdb; $AddtoCats = array(); $AddCounter=0; $UpdateCounter=0; $sc=array(); $PARENTCAT=""; $arrayParents = array();
	
	$imcats = array();
	
	// FIRST SETUP THE CATEGORIES MAKING IMPORT EASIER
	$SQL = "SELECT ".$wpdb->prefix."dfr_shop_cats.*  FROM ".$wpdb->prefix."dfr_shop_cats ORDER BY parent ASC"; 	
	$results = (array)$wpdb->get_results($SQL,ARRAY_A);
 
	$a=0;
	foreach ($results as $cats) {
	
	$imcats[$a]['id'] 		=  $cats['id']; // <-- parent id
	$imcats[$a]['name'] 	=  $cats['name']; // <-- name
	$imcats[$a]['parent'] 	=  $cats['parent']; // <-- name
	
	
		// ITS A PARENT CATEGORY
		if($cats['parent'] == 0){
		
			$catzID = $this->wp_create_category1($cats['name']); // CREAT THE PARENT CAT
			 		 
			$imcats[$a]['parent-id'] =  $catzID;
			
			$arrayParents[$cats['id']] = $catzID;
		
					
		}else{		 
		
			// LEVEL 1 SUB CAT
			if($arrayParents[$cats['parent']] != ""){
			
				// ADD THE SUB CATEGORY
				$catid = $this->wp_create_category1($cats['name'], $arrayParents[$cats['parent']] );
				
				$arrayParents[$cats['id']] = $catid;
