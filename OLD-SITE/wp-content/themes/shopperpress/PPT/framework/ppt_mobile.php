...
function start(){
	...
	if(isset($_POST['action']) && strlen($_POST['form']['name']) > 2){
...
function _check(){
		...
		// CHECK FOR IPAD 		
		if (strpos(strtolower($agent), "ipad") === false) { }else{ return "ipad";} 
		// CHECK FOR FACEBOOK
...
ul.paginationD01 li a:hover,ul.paginationD01 li a.current{color:#893A00;text-shadow:0px 1px #FFEF42;border-color:#FFA200;background:#FFC800;background:-moz-linear-gradient(top,#FFFFFF 1px,#FFEA01 1px,#FFC800);background:-webkit-gradient(linear,0 0,0 100%,color-stop(0.02,#FFFFFF),color-stop(0.02,#FFEA01),color-stop(1,#FFC800));}
 ...
 ul.paginationD10 li a:hover,ul.paginationD10 li a.current{text-shadow:0px 1px #3C3C3C;border-color:#202020;background:#525252;background:-moz-linear-gradient(top,#9F9F9F 1px,#6C6C6C 1px,#525252);background:-webkit-gradient(linear,0 0,0 100%,color-stop(0.02,#9F9F9F),color-stop(0.02,#6C6C6C),color-stop(1,#525252));}
	...
	$STRING .= '<script src="'.PPT_THEME_URI.'/PPT/player/mediaelement-and-player.min.js" type="text/javascript"></script>';
	$STRING .= '<link rel="stylesheet" type="text/css" href="'.PPT_THEME_URI.'/PPT/player/mediaelementplayer.min.css" media="screen" />';
...
function _footer(){
	...
	$STRING ="";
	$STRING .= stripslashes(get_option("advertising_mobile"));
...
function _body(){
	...
	switch($_GET['show']){
		...
		case "search":{
			...
			$page1 = explode("/page/",$_SERVER['REQUEST_URI']);
			if(isset($page1[1])){
			...
			if(isset($_GET['paged'])){ $paged =$_GET['paged'];  }else{ $paged = 1; }
			 ...
			 $qqf = $query_string.'numberposts=25&post_type=post&paged='.$paged.$extra;
			 
			$a  = query_posts($qqf); //&order=DESC&orderby=date&showposts=8
		...
		case "single": {		
			...
			if(strtolower(PREMIUMPRESS_SYSTEM) == "moviepress"){	
				...
				$GLOBALS['videoimage'] 		= get_post_meta($post->ID, "image", true);
				$GLOBALS['embed'] 		= get_post_meta($post->ID, "embed", true);
								
				if($GLOBALS['MoveID'] == ""){
				$GLOBALS['MoveID'] = $GLOBALS['video'];
				if($GLOBALS['MoveID'] == ""){
				$GLOBALS['MoveID'] = $GLOBALS['videoimage'];
				}
				}
				...
				// if(!is_array($GLOBALS['access']) || $GLOBALS['access'] =="" || $GLOBALS['access'][0] == "00" || $GLOBALS['access'][0] == "" || in_array($GLOBALS['membershipPackage'],$GLOBALS['access'])){ 
					...
					}else{ $extra .= $ThemeDesign->VIDEOPLAYER($GLOBALS['MoveID'],true);   }
					$extra .='</div>';
				...
				//$extra .='This video is only for ';
				 ...
				 //foreach($GLOBALS['access'] as $key){ 
				 
				 //$extra .= strip_tags($PACKAGE_OPTIONS[$key]['name'])." "; }  
			...
			foreach($post_categories as $c){
				...
				$catSTRING .= "<a href='".HOME_URI."?show=search&catID=".$c."' style='text-decoration:underline;'>".$cat->name."</a> ";
			}
			...
			if(strlen($catSTRING) > 1){				
				$STRING .='<p>Listed in '.$catSTRING.' </p>';
			}
			
			$STRING .= '<p>Created: '.date('l jS \of F Y', strtotime($post->post_date)).'</p>';
			
			if(is_numeric($hits)){
			$STRING .= '<p>Visitors: '.$hits.'</p>';
			}
				 
			$STRING .= $PPTDesign->CustomFields($post,get_option("customfielddata"),true).'<div class="clear"></div>';
		...
		default: {
			...
			<center><a href="'.HOME_URI.'/?hide_mobile_view=1" style="text-decoration:underline; font-size:11px;">Show Full Website</a></center>
...
return $STRING;
	...
	function Pages($type=0){
		...
		// CHECK IF IT HAS A PAGE TEMPLATE ATTACHED, IF IT DOES, HIDE IT
		
		$PageTemplate = get_post_meta($Page->ID,'_wp_page_template',true);
		 
		if($PageTemplate != "default"){ continue; }
	...
	switch(strtolower(PREMIUMPRESS_SYSTEM)){
		...
		case "shopperpress": {  
			...
			$link 	= premiumpress_link($post->ID,true);
			$STRING .= '<div style="float:right"><a href="'.$link.'" target="_blank" rel="nofollow" style="border-bottom:1px #666 dotted">Visit Website</a> &nbsp;&nbsp;</div>';
			}
