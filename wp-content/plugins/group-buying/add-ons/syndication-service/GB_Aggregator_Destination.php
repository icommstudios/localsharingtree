...
class GB_Aggregator_Destination extends GB_Aggregator_Plugin {
	...
	private function create_post( $uri ) {
		...
		do_action( 'gb_log', __CLASS__ . '::' . __FUNCTION__ . ' - Creating post for syndicated deal', $deal );
	...
	private function update_post( $post_id, $uri ) {
		...
		// Logging
		do_action( 'gb_log', __CLASS__ . '::' . __FUNCTION__ . ' - Updating post for syndicated deal - Post ID', $post_id );
		do_action( 'gb_log', __CLASS__ . '::' . __FUNCTION__ . ' - Updating post for syndicated deal - DEAL', $deal );
