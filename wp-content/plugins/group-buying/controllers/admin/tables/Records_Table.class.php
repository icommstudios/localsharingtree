...
class Group_Buying_Records_Table extends WP_List_Table {
	protected static $post_type = Group_Buying_Record::POST_TYPE;
	...
	function extra_tablenav( $which ) {
		...
		if ( 'top' == $which && !is_singular() ) {
			...
			if ( is_object_in_taxonomy( self::$post_type, Group_Buying_Record::TAXONOMY ) ) {
				$term_id = ( isset( $_GET[Group_Buying_Record::TAXONOMY] ) ) ? $_GET[Group_Buying_Record::TAXONOMY] : 0 ;
				$dropdown_options = array(
					...
					'orderby' => 'name',
					'selected' => $term_id,
					'name' => Group_Buying_Record::TAXONOMY
				);
			...
			if ( count( $this->items ) > 0 ) {
				if ( isset( $_GET[Group_Buying_Record::TAXONOMY] ) && $_GET[Group_Buying_Record::TAXONOMY] ) {
					$term = get_term( $_GET[Group_Buying_Record::TAXONOMY], Group_Buying_Record::TAXONOMY );
				...
				printf( '<input type="hidden" name="%s" value="%s" />', Group_Buying_Records::RECORD_PURGE_NONCE, wp_create_nonce( Group_Buying_Records::RECORD_PURGE_NONCE ) );
			}
	...
	function column_default( $item, $column_name ) {
		switch ( $column_name ) {
		default:
			return apply_filters( 'gb_mngt_record_column_'.$column_name, $item ); // do action for those columns that are filtered in
		}
	}
	...
	function column_type( $item ) {
		$record = Group_Buying_Record::get_instance( $item->ID );
	...
	function column_data( $item ) {
		...
		if ( $data != '' ) {
			?>
				<a href="#TB_inline?width=900&height=600&inlineId=data_id_<?php echo $item->ID ?>" class="thickbox button" title="<?php echo $item->post_title ?> <?php gb_e('Data') ?>"><?php gb_e('View Data') ?></a>
	...
	function get_columns() {
		$columns = array(
			...
			'title'  => gb__('Subject'),
			'data'  => gb__('Data'),
			'associate'  => gb__('Associated Content'),
			'type' => gb__('Type')
		);
		return apply_filters( 'gb_mngt_record_columns', $columns );
	}
	...
	function get_sortable_columns() {
		...
		return apply_filters( 'gb_mngt_record_sortable_columns', $sortable_columns );
	}
	...
	function get_bulk_actions() {
		$actions = array();
		return apply_filters( 'gb_mngt_records_bulk_actions', $actions );
	}
	...
	function prepare_items() {
		...
		// Filter by taxonomy
		if ( isset( $_GET[Group_Buying_Record::TAXONOMY] ) && $_GET[Group_Buying_Record::TAXONOMY] && $_GET[Group_Buying_Record::TAXONOMY] != '' ) {
			$tax_query = array(
					'tax_query' => array(
							array(
								'taxonomy' => Group_Buying_Record::TAXONOMY,
								'field' => 'id',
								'terms' => $_GET[Group_Buying_Record::TAXONOMY]
							)
						)
				);
		 ...
		 */
		$this->items = apply_filters( 'gb_mngt_records_items', $records->posts );
