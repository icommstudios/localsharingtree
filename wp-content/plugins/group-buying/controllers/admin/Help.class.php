...
class Group_Buying_Help extends Group_Buying_Controller {
	...
	const NONCE = 'gbs_save_api_key_nonce';
	...
	public static function init() {
		...
		if ( is_admin() ) {
			...
			add_action( 'wp_ajax_gbs_save_api_key',  array( get_class(), 'save_api_key' ), 10, 0 );
		}
	...
	public function save_api_key() {
		// check for nonce
		if ( !isset( $_REQUEST['save_api_key_nonce'] ) )
			die( 'Forget something?' );

		// Validate nonce
		$nonce = $_REQUEST['save_api_key_nonce'];
		if ( !wp_verify_nonce( $nonce, self::NONCE ) )
			die ( 'Not going to fall for it!');
		
		update_option( 'group_buying_site_api_key', $_REQUEST['api_key'] );
	}
