...
class Group_Buying_Paypal_AP extends Group_Buying_Offsite_Processors {
	...
	public static function get_preapproval( Group_Buying_Checkouts $checkout ) {
		...
		do_action( 'gb_log', __CLASS__ . '::' . __FUNCTION__ . ' - PayPal AP Preapproval Response', $response );
	...
	public static function get_preapproval_detail( Group_Buying_Checkouts $checkout, Group_Buying_Purchase $purchase ) {
		...
		do_action( 'gb_log', __CLASS__ . '::' . __FUNCTION__ . ' - PayPal AP PreapprovalDetails Response', $response );
		...
		if ( $payment->get_payment_method() == self::get_payment_method() && $payment->get_status() != Group_Buying_Payment::STATUS_COMPLETE ) {
			...
			if ( isset( $data['api_response']['preapproval_key'] ) && $data['api_response']['preapproval_key'] ) {
				...
				if ( $items_to_capture ) {
					...
					do_action( 'gb_log', __CLASS__ . '::' . __FUNCTION__ . ' - PayPal AP items_to_capture', $items_to_capture );
					...
					foreach ( $items_to_capture as $deal_id => $amount ) {
						...
						do_action( 'gb_log', __CLASS__ . '::' . __FUNCTION__ . ' - PayPal AP Capture Cal Pay - deal_id', $deal_id );
						do_action( 'gb_log', __CLASS__ . '::' . __FUNCTION__ . ' - PayPal AP Capture Cal Pay - response', $response );
			...
			// Check if the payment date is older than preapproval period plus 7 days
			if ( get_post_time( 'U', FALSE, $payment->get_id() ) < ( time() - ( apply_filters( 'gb_paypal_ap_endingperiod_for_preapproval', 7776000 )+(60*60*24*7) ) ) ) {
	...
	private function get_payment( $payment, $deal_id, $amount, $qty = 1, $pre_app_key = '', $tracking_id = 0 ) {
		...
		if ( !apply_filters( 'gb_paypal_ap_use_parallel_payments', FALSE ) ) {
			...
			do_action( 'gb_log', __CLASS__ . '::' . __FUNCTION__ . ' - PayPal AP Using Parallel Payments' );
		...
		do_action( 'gb_log', __CLASS__ . '::' . __FUNCTION__ . ' - PayPal AP Get Payment - call', $fields );

		// Make the call
		$response = self::remote_post( 'Pay', $fields );
		do_action( 'gb_log', __CLASS__ . '::' . __FUNCTION__ . ' - PayPal AP Get Payment Response', $response );
	...
	private function remote_post( $method_name = 'Preapproval', $post_array = array() ) {
		...
		do_action( 'gb_log', __CLASS__ . '::' . __FUNCTION__ . ' - PayPal AP Remote Post', $response );
	...
	public static function show_adaptive_meta_box( Group_Buying_Deal $deal, $post, $metabox ) {
		...
		include dirname( __FILE__ ) .  '/views/meta-boxes/deal-adaptive-payments.php';
	}
