...
class Group_Buying_Hybrid_Payment_Processor extends Group_Buying_Payment_Processors {
	...
	protected function __construct() {
		...
		add_filter( 'gb_checkout_panes_'.Group_Buying_Checkouts::PAYMENT_PAGE, array( $this, 'before_payment_pane' ), -10, 2 );
	...
	public function before_payment_pane() {
		if ( empty($this->current_payment_processor) ) {
			return; // not sure how this can be, but let's be safe
		}
		$enabled_processors = self::enabled_processors();
		$credit_processors = array();
		foreach ( $enabled_processors as $p ) {
			if ( self::is_cc_processor($p) ) {
				$credit_processors[] = $p;
			}
		}
		if ( count($credit_processors) > 0 && !self::is_cc_processor($this->current_payment_processor) ) {
			// A non-credit processor has been loaded, probably to processes a failed submission.
			// We need to make sure that the credit processor still loads its payment pane
			$this->load_processor( reset($credit_processors) );

			// And remove the pane from the offsite processor
			remove_filter( 'gb_checkout_panes_'.Group_Buying_Checkouts::PAYMENT_PAGE, array( $this->current_payment_processor, 'payment_pane' ), 10, 2 );
			// Also make sure it doesn't take over the "Review" button
			remove_filter( 'gb_checkout_payment_controls', array( $this->current_payment_processor, 'payment_controls' ), 10, 2 );
			remove_filter( 'gb_checkout_payment_controls', array( $this->current_payment_processor, 'payment_controls' ), 20, 2 );
		...
		if ( count($enabled) > 1 ) {
			...
			foreach ( $enabled as $class ) {
				if ( self::is_cc_processor($class) ) {
					if ( self::is_cc_processor($this->current_payment_processor) ) {
