 ...
 * @return string
 */
function gb_get_accounts_report_url( $csv = FALSE, $post_id = 0 ) {
	if ( isset( $_GET['id'] ) ) {
		$post_id = $_GET['id'];
	}
 ...
 * @return string
 */
function gb_get_credits_report_url( $post_id = 0, $csv = FALSE ) {
	if ( isset( $_GET['id'] ) ) {
		$post_id = $_GET['id'];
	}
	...
	$report = Group_Buying_Reports::get_instance( 'credits' );
	if ( $csv ) {
		return apply_filters( 'gb_get_credits_report_url', add_query_arg( array( 'report' => 'credits', 'id' => $post_id ), $report->get_csv_url() ) );
	}
	return apply_filters( 'gb_get_credits_report_url', add_query_arg( array( 'report' => 'credits', 'id' => $post_id ), $report->get_url() ) );
}

/**
 * Print credits report url
 * @see gb_get_credits_report_url()
 * @param integer $post_id Merchant ID
 * @return string           url
 */
function gb_credits_report_url( $post_id = 0 ) {
	echo apply_filters( 'gb_credits_report_url', gb_get_credits_report_url( $post_id ) );
}

/**
 * Print credits report link <a>
 * @see gb_get_credits_report_url()
 * @param integer $post_id Merchant ID
 * @return string           link
 */
function gb_credits_report_link( $post_id = 0 ) {
	$link = '<a href="'.gb_get_credits_report_url( $post_id ).'" class="report_button">'.gb__( 'Purchase History' ).'</a>';
	echo apply_filters( 'gb_credits_report_url', $link );
}
 ...
 * @return string           url
 */
function gb_credits_report_csv_url( $post_id = 0 ) {
	echo apply_filters( 'gb_credits_report_url', gb_get_credits_report_url( $post_id, true ) );
}
 ...
 * @return string
 */
function gb_get_credit_purchases_report_url( $post_id = 0, $csv = FALSE ) {
	if ( isset( $_GET['id'] ) ) {
		$post_id = $_GET['id'];
	}
	...
	$report = Group_Buying_Reports::get_instance( 'credit_purchases' );
	if ( $csv ) {
		return apply_filters( 'gb_get_credit_purchases_report_url', add_query_arg( array( 'report' => 'credit_purchases', 'id' => $post_id ), $report->get_csv_url() ) );
	}
	return apply_filters( 'gb_get_credit_purchases_report_url', add_query_arg( array( 'report' => 'credit_purchases', 'id' => $post_id ), $report->get_url() ) );
}

/**
 * Print credit purchases report url
 * @see gb_get_credit_purchases_report_url()
 * @param integer $post_id Merchant ID
 * @return string           url
 */
function gb_credit_purchases_report_url( $post_id = 0 ) {
	echo apply_filters( 'gb_credit_purchases_report_url', gb_get_credit_purchases_report_url( $post_id ) );
}

/**
 * Print credit purchases report link <a>
 * @see gb_get_credit_purchases_report_url()
 * @param integer $post_id Merchant ID
 * @return string           link
 */
function gb_credit_purchases_report_link( $post_id = 0 ) {
	$link = '<a href="'.gb_get_credit_purchases_report_url( $post_id ).'" class="report_button">'.gb__( 'Purchase History' ).'</a>';
	echo apply_filters( 'gb_credit_purchases_report_url', $link );
}
 ...
 * @return string           url
 */
function gb_credit_purchases_report_csv_url( $post_id = 0 ) {
	echo apply_filters( 'gb_credit_purchases_report_url', gb_get_credit_purchases_report_url( $post_id, true ) );
}
...
function gb_get_current_report_csv_download_url() {
	...
	$rep = $_GET['report'];
	$report = Group_Buying_Reports::get_instance( $rep );
