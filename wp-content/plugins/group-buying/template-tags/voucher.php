...
function gb_on_voucher_active_page() {
	return get_query_var( Group_Buying_Vouchers::FILTER_QUERY_VAR ) == Group_Buying_Vouchers::FILTER_ACTIVE_QUERY_VAR;
}
...
function gb_on_voucher_expired_page() {
	return get_query_var( Group_Buying_Vouchers::FILTER_QUERY_VAR ) == Group_Buying_Vouchers::FILTER_EXPIRED_QUERY_VAR;
}
...
function gb_on_voucher_used_page() {
	return get_query_var( Group_Buying_Vouchers::FILTER_QUERY_VAR ) == Group_Buying_Vouchers::FILTER_USED_QUERY_VAR;
}
...
function gb_voucher_locations( $voucher_id = 0 ) {
	...
	$out = '';
	if ( empty( $locations ) && gb_has_merchant( gb_get_vouchers_deal_id() ) ) {
		$merchant_id = gb_get_merchant_id( gb_get_vouchers_deal_id() );
		if ( gb_has_merchant_name( $merchant_id ) ) {
			$locations[] = gb_get_merchant_name( $merchant_id );
		}
		if ( gb_has_merchant_street( $merchant_id ) ) {
			$locations[] = gb_get_merchant_street( $merchant_id );
		}
		if ( gb_has_merchant_city( $merchant_id ) ) {
			$city_line = gb_get_merchant_city( $merchant_id );
		}
		if ( $city_line && ( gb_has_merchant_state( $merchant_id ) || gb_has_merchant_zip( $merchant_id ) ) ) {
			$city_line .= ', ';
		}
		$city_line .= gb_get_merchant_state( $merchant_id );
		$city_line = rtrim( $city_line ) . ' ' . gb_get_merchant_zip( $merchant_id );
