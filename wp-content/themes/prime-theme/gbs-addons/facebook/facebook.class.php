...
?>
		...
		public static function connect() {
			...
			if ( self::is_facebook_logged_in() ) {
				...
				// fb user object
				do_action( 'gb_log', 'fb connect user_data', $user_data  );
				...
				if ( empty( $wp_user_id ) ) {
					...
					do_action( 'gb_log', 'fb connect - wp_user_id: ', $wp_user_id );
				}
		...
		public static function button( $button_text = 'Login with Facebook' ) {
			...
			if ( !empty( self::$fb_app_id ) && !empty( self::$fb_sec ) ) {
				return apply_filters( 'gb_fb_add_login_button', '<span class="fb-login-button" data-show-faces="false" data-width="200" data-max-rows="1" onlogin="fbActionConnect()">' . self::__( $button_text ) . '</span>' );
			}
		}
		...
		public static function get_facebook_authorization_request() {
			...
			do_action( 'gb_log', 'get_facebook_authorization_request - signed request', $signed_request );
			...
			if ( !is_null( $signed_request ) ) {
				...
				do_action( 'gb_log', 'get_facebook_authorization_request - response: ', $access_token_response  );
